...
public class UserAuthService {
  ...
  private boolean isFirstMiniappLogin(WechatIdentity identity) {
    ...
    if (userWechatAuthMapper.findBySourceAndOpenid(WECHAT_SOURCE_MINIAPP, identity.openid()) != null) {
      return false;
    }
    if (userWechatAuthMapper.findByOpenid(identity.openid()) != null) {
      return false;
    }

    String unionid = normalizeText(identity.unionid(), "");
    if (!unionid.isBlank() && userWechatAuthMapper.findByUnionid(unionid) != null) {
      return false;
    }
    return true;
  }
  ...
  private WechatIdentity exchangeMiniappIdentity(String code) {
    ...
    try {
      ...
      if (response.statusCode() < 200 || response.statusCode() >= 300) {
        throw new BusinessException("瀵邦喕淇婇惂璇茬秿閺堝秴濮熸潻鏂挎礀瀵倸鐖堕悩鑸碘偓? " + response.statusCode());
      }
      ...
      if (errCode instanceof String text && !text.isBlank() && !"0".equals(text)) {
        ...
        throw new BusinessException("Mini-program login failed: " + errMsg);
      }
      if (errCode instanceof Number number && number.intValue() != 0) {
        String errMsg = String.valueOf(payload.getOrDefault("errmsg", "unknown"));
        throw new BusinessException("鐏忓繒鈻兼惔蹇曟瑜版洖銇戠拹? " + errMsg);
      }

      Object openid = payload.get("openid");
      if (!(openid instanceof String openidValue) || openidValue.isBlank()) {
        throw new BusinessException("鐏忓繒鈻兼惔蹇曟瑜版洘婀潻鏂挎礀 openid");
      }
      ...
      throw new BusinessException("获取微信登录身份失败，缺少 openid");
    }
  }
