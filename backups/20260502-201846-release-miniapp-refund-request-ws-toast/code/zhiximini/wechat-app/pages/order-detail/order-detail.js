...
const { normalizeOrder, getOrderStatusText, getOrderStatusClass } = require('../../utils/order.js');
...
Page({
      ...
      .then((res) => {
        ...
        this.setData({
          order: {
            ...
            canRefundRequest: this.canRefundRequest(item),
  ...
  canRefundRequest(order) {
    if (!order) return false;
    if (!['PAID', 'SHIPPED', 'COMPLETED'].includes(order.status)) return false;
    if (['PROCESSING', 'SUCCESS'].includes(order.refundStatus)) return false;
    if (order.refundRequestStatus === 'PENDING') return false;
    return true;
  },

  openRefundPopup() {
    this.setData({ showRefundPopup: true, refundReason: '' });
  },

  closeRefundPopup() {
    this.setData({ showRefundPopup: false, refundReason: '' });
  },
  ...
  submitRefundRequest() {
    ...
    if (reason.length > 200) {
      wx.showToast({ title: '退款理由最长200字', icon: 'none' });
      return;
    }
    ...
    request({
      ...
      .then(() => {
        ...
        this.setData({ showRefundPopup: false, refundReason: '', submittingRefund: false });
        this.loadDetail(orderId);
      })
      .catch((error) => {
        const msg = (error && error.data && error.data.message) || (error && error.message) || '提交失败';
