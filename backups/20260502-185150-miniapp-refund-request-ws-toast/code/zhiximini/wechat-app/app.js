...
const TRADE_MANAGE_PENDING_ORDER_ID_STORAGE_KEY = 'tradeManagePendingOrderId';
...
App({
  globalData: {
    ...
    tradeManagePendingOrderId: null
  },
  ...
  onLaunch(options) {
    ...
    this.hydrateTradeManagePendingOrderId();
  ...
  hydrateTradeManagePendingOrderId() {
    const current = this.normalizeTradeManagePendingOrderId(this.globalData.tradeManagePendingOrderId);
    if (current) {
      this.globalData.tradeManagePendingOrderId = current;
      return current;
    }

    const stored = this.normalizeTradeManagePendingOrderId(wx.getStorageSync(TRADE_MANAGE_PENDING_ORDER_ID_STORAGE_KEY));
    this.globalData.tradeManagePendingOrderId = stored;
    return stored;
  },

  setTradeManagePendingOrderId(orderId) {
    const normalized = this.normalizeTradeManagePendingOrderId(orderId);
    this.globalData.tradeManagePendingOrderId = normalized;
    if (normalized) {
      wx.setStorageSync(TRADE_MANAGE_PENDING_ORDER_ID_STORAGE_KEY, normalized);
      return normalized;
    }
    wx.removeStorageSync(TRADE_MANAGE_PENDING_ORDER_ID_STORAGE_KEY);
    return null;
  },

  peekTradeManagePendingOrderId() {
    return this.hydrateTradeManagePendingOrderId();
  },

  clearTradeManagePendingOrderId() {
    this.globalData.tradeManagePendingOrderId = null;
    wx.removeStorageSync(TRADE_MANAGE_PENDING_ORDER_ID_STORAGE_KEY);
  },
  ...
  normalizeTradeManagePendingOrderId(value) {
