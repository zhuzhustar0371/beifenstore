      ...
      <div v-else-if="withdrawals.length > 0 && isMobileAdmin" class="cashback-mobile-list">
        <article v-for="withdrawal in withdrawals" :key="withdrawal.id" class="cashback-mobile-card">
          <div class="cashback-mobile-card__top">
            <div class="flex min-w-0 items-center gap-phi-2">
              <img
                ...
                :alt="`${displayUserName(withdrawal)}澶村儚`"
          ...
          <div class="cashback-mobile-card__grid">
            <div>
              <p class="cashback-mobile-card__label">鐢宠閲戦</p>
              <p class="cashback-mobile-card__value">楼{{ formatMoney(withdrawal.requestedAmount || withdrawal.amount) }}</p>
            </div>
            <div>
              <p class="cashback-mobile-card__label">寤鸿鎵瑰噯</p>
              <p class="cashback-mobile-card__value">楼{{ formatMoney(withdrawal.suggestedAmount || withdrawal.readyAmount || 0) }}</p>
            </div>
            <div>
              <p class="cashback-mobile-card__label">妯″紡</p>
              <p class="cashback-mobile-card__meta">{{ withdrawalApplyModeText(withdrawal.applyMode) }}</p>
            </div>
            <div>
              <p class="cashback-mobile-card__label">鏉ユ簮</p>
          ...
          <div class="cashback-mobile-card__actions">
            ...
            >
              {{ approvingWithdrawalId === withdrawal.id ? "鎵瑰噯涓?.." : "鎵瑰噯鎻愮幇" }}
            </button>
    ...
    <GlassCard>
      ...
      <div v-else-if="cashbacks.length > 0 && isMobileAdmin" class="cashback-mobile-list">
        <article v-for="cashback in cashbacks" :key="cashback.id" class="cashback-mobile-card">
          <div class="cashback-mobile-card__top">
            <div class="flex min-w-0 items-center gap-phi-2">
              <img
                ...
                :alt="`${displayUserName(cashback)}澶村儚`"
                class="h-10 w-10 rounded-full border border-white/25 object-cover shadow-glass-soft"
                referrerpolicy="no-referrer"
          ...
          <div class="cashback-mobile-card__grid">
            <div>
              <p class="cashback-mobile-card__label">绫诲瀷</p>
              <p class="cashback-mobile-card__meta">{{ cashbackTypeText(cashback.type) }}</p>
            </div>
            <div>
              <p class="cashback-mobile-card__label">閲戦</p>
              <p class="cashback-mobile-card__value">
                <span v-if="cashback.earlyWithdrawal" class="early-warning">鈿狅笍 </span>
                楼{{ formatMoney(cashback.amount) }}
              </p>
            </div>
            <div>
              <p class="cashback-mobile-card__label">璁㈠崟缂栧彿</p>
              <p class="cashback-mobile-card__meta">{{ cashback.orderId || "-" }}</p>
            </div>
            <div>
              <p class="cashback-mobile-card__label">鍒涘缓鏃堕棿</p>
              <p class="cashback-mobile-card__meta">{{ cashback.createdAt || "-" }}</p>
            </div>
          </div>

          <div class="cashback-mobile-card__section">
            <p class="cashback-mobile-card__label">澶囨敞</p>
            <p class="cashback-mobile-card__meta">{{ cashback.remark || "-" }}</p>
            <p v-if="cashback.transferFailReason" class="cashback-mobile-card__danger">{{ cashback.transferFailReason }}</p>
          </div>

          <div class="cashback-mobile-card__section">
            <p class="cashback-mobile-card__label">寰俊鎵规 / 鏄庣粏</p>
          ...
          <div class="cashback-mobile-card__actions">
            ...
            >
              {{ syncingId === cashback.id ? "鍚屾涓?.." : (cashback.outBatchNo ? "鍚屾鐘舵€?" : "涓嶅彲鎿嶄綔") }}
            </button>
            <button
