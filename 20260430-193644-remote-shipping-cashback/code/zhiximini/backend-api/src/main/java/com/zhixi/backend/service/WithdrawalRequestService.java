...
public class WithdrawalRequestService {
  ...
  public WithdrawalRequest createUserRequest(Long userId, String idempotencyKey, String applyMode) {
    ...
    if (idempotencyKey != null && !idempotencyKey.isEmpty()) {
      ...
      if (existing != null) {
        throw new BusinessException("Duplicate withdrawal request");
      }
    }
    ...
    if (locked == null || !locked) {
      throw new BusinessException("A withdrawal request is already being processed");
    }
    ...
    try {
      ...
      if (pendingCount >= MAX_PENDING_PER_USER) {
        throw new BusinessException("Too many pending withdrawal requests");
      }
      ...
      if (selectedRecords.isEmpty()) {
        throw new BusinessException("No cashback records available for " + normalizedApplyMode);
      }
      ...
      if (debtTotal.compareTo(BigDecimal.ZERO) > 0) {
        ...
        if (netAmount.compareTo(BigDecimal.ZERO) <= 0) {
          throw new BusinessException("Cashback is not enough to deduct pending debt");
        }
      ...
      if (confirmedAmount.compareTo(BigDecimal.ZERO) <= 0) {
        ...
        throw new BusinessException("No cashback records after locking");
      }
