...
### Invite cashback immediate-withdrawal application fix

- Time: 2026-04-27 19:16:30 +08:00
- Operator: Codex
- Goal:
  - Keep the existing rule that invite cashback is generated only after every 3 invitees complete first paid orders
  - Fix the user withdrawal application amount so generated invite cashback is counted immediately
  - Keep backend payout restricted by the existing 7-day maturity window
- User-approved business rule:
  - `满三人才返现`
  - Invite cashback can enter the user's withdrawal application amount immediately after generation
  - Backend/admin payout still follows the 7-day maturity rule
- Safety and backup:
  - Local full backup created before edits:
    - `G:\store\20260427-172456-invite-withdraw-immediate-apply`
  - Git backup pushed to:
    - `git@github.com:zhuzhustar0371/beifenstore.git`
    - commit `81756d9`
  - Note:
    - GitHub push protection blocked the first raw backup attempt because historical backup docs contained sensitive config snapshots
    - The final remote backup used a sanitized backup worktree for push, while the untouched full local backup was preserved as the rollback baseline
- Files changed:
  - `backend-api/src/main/java/com/zhixi/backend/mapper/CashbackRecordMapper.java`
    - Added `findByWithdrawalRequestId(Long requestId)` for request-bound cashback lookup
    - Changed `findWithdrawableByUserId(Long userId)` so `INVITE_BATCH` is immediately withdraw-requestable while other cashback types still require `eligible_at <= NOW()`
  - `backend-api/src/main/java/com/zhixi/backend/service/AdminManageService.java`
    - Added 7-day maturity guard to `transferCashback(Long cashbackId)`
    - Changed withdrawal approval flow to validate linked cashback maturity before moving the request to payout
    - Added explicit admin error messaging with ready amount, pending amount, and earliest payout time when a request still contains immature cashback
    - Skipped re-triggering cashback records already in payout flow (`WAIT_USER_CONFIRM`, `PROCESSING`, `TRANSFERING`, `CANCELING`, `TRANSFERRED`)
  - `wechat-app/pages/cashback/cashback.js`
    - Updated withdrawable calculation so `INVITE_BATCH` can be applied immediately
  - `wechat-app/pages/user/user.js`
    - Updated balance calculation to use the same withdrawable rule as the cashback detail page
  - `zhixi-website/admin-frontend/src/views/CashbacksPage.vue`
    - Added maturity hint text per cashback row
    - Disabled payout action before 7-day maturity
    - Added a frontend-side precheck message showing the earliest payout time
- Local verification:
  - `backend-api`: `mvn -q -DskipTests package` passed
  - `zhixi-website/admin-frontend`: `npm run build` passed
  - `wechat-app/pages/cashback/cashback.js`: `node --check` passed
  - `wechat-app/pages/user/user.js`: `node --check` passed
- Deployment status:
  - Local changes only
  - No cloud build executed
  - No release executed
