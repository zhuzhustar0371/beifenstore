...
public class PayNotifyController {
  ...
  public ResponseEntity<Map<String, String>> handleTransferNotify(HttpServletRequest request) {
    try {
      RequestParam requestParam = buildRequestParam(request);
      NotificationParser parser = new NotificationParser((NotificationConfig) wechatPayService.getConfig());
      Map<?, ?> transfer = parser.parse(requestParam, Map.class);
      CashbackRecord record = cashbackService.updateTransferFromWechatNotification(transfer);

      log.info(
          "Received WeChat merchant transfer callback, cashbackId={}, outDetailNo={}, status={}",
          record.getId(),
          record.getOutDetailNo(),
          record.getStatus()
      );

      return ResponseEntity.ok(successBody());
    } catch (Exception e) {
      log.error("Failed to process WeChat merchant transfer callback", e);
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(failBody());
    }
  }
