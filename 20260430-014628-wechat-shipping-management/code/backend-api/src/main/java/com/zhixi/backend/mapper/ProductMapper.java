...
public interface ProductMapper {
  String PRODUCT_COLUMNS = "id,name,price,description,main_image_url AS imageUrl,COALESCE(sales_count, 0) AS salesCount,(status = 1) AS active,(COALESCE(is_featured, 0) = 1) AS featured,created_at AS createdAt";
  ...
  @Insert("INSERT INTO products(name, sub_title, description, main_image_url, price, status, is_featured, sort_no, created_at, updated_at) " +
      "VALUES(#{name}, #{name}, #{description}, #{imageUrl}, #{price}, CASE WHEN #{active} THEN 1 ELSE 0 END, CASE WHEN #{featured} THEN 1 ELSE 0 END, 0, NOW(), NOW())")
