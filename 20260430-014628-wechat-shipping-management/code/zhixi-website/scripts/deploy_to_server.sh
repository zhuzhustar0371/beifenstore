...
SERVER_IP="$2"
DEPLOY_ROOT="$3"
LEGACY_ADMIN_ROOT="/home/ubuntu/apps/manager-backend"
LEGACY_ADMIN_DIST="$LEGACY_ADMIN_ROOT/dist"
...
ssh "$SSH_USER@$SERVER_IP" "set -euo pipefail; \
  ...
  if [ '$DEPLOY_ROOT' = '/home/ubuntu/zhixi' ]; then \
    sudo mkdir -p '$LEGACY_ADMIN_ROOT/backups'; \
    if sudo test -d '$LEGACY_ADMIN_DIST' && [ \"\$(sudo find '$LEGACY_ADMIN_DIST' -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)\" ]; then \
      sudo cp -a '$LEGACY_ADMIN_DIST' '$LEGACY_ADMIN_ROOT/backups/dist-$TIMESTAMP'; \
    fi; \
    sudo rm -rf '$LEGACY_ADMIN_DIST'; \
    sudo mkdir -p '$LEGACY_ADMIN_ROOT'; \
    sudo cp -a '$DEPLOY_ROOT/current/admin-frontend/dist' '$LEGACY_ADMIN_DIST'; \
    sudo chmod -R a+rX '$LEGACY_ADMIN_DIST' || true; \
    chmod o+x /home/ubuntu || true; \
  fi"
