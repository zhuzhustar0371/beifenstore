      ...
      <div v-else-if="orders.length > 0 && isMobileAdmin" class="orders-mobile-list">
        <article v-for="order in orders" :key="order.id" class="orders-mobile-card">
          ...
          <div class="orders-mobile-card__actions">
            ...
            <button v-else class="btn-inline" :disabled="shippingId === order.id || !canShip(order)" @click="onShip(order)">
              {{ shippingId === order.id ? "发货中..." : (canShip(order) ? "发货" : "不可发货") }}
            </button>
            <template v-if="order.refundRequestStatus === 'PENDING'">
              <button class="btn-inline" :disabled="approvingId === order.id" @click="openApproveDialog(order)">
                {{ approvingId === order.id ? "处理中..." : "批准退款" }}
              </button>
              <button class="btn-inline !border-red-400/40 !text-red-200" :disabled="rejectingId === order.id" @click="onReject(order)">
                {{ rejectingId === order.id ? "处理中..." : "驳回申请" }}
              </button>
            </template>
            <button v-else class="btn-inline" :disabled="refundingId === order.id || !canRefund(order)" @click="openRefundDialog(order)">
