...
function isAuthMessage(message) {
  ...
  return /璇峰厛鐧诲綍|鐧诲綍鐘舵€佸凡杩囨湡|閲嶆柊鐧诲綍|鐢ㄦ埛涓嶅瓨鍦?.test(message);
}
...
function showAuthToast(message) {
  ...
  lastAuthToastAt = now;
  wx.showToast({ title: message || '璇峰厛鐧诲綍', icon: 'none' });
}
...
function request(options) {
  ...
  return new Promise((resolve, reject) => {
    wx.request({
      ...
      success: (res) => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          if (res.data && typeof res.data === 'object' && res.data.success === false) {
            const message = extractMessage(res.data, '璇锋眰澶辫触');
        ...
        if (res.statusCode === 401 || isAuthMessage(message)) {
          rejectAuthFailure(message || '璇峰厛鐧诲綍', res, reject);
          return;
        }
      ...
      fail: (err) => {
        wx.showToast({ title: '缃戠粶閿欒', icon: 'none' });
        reject(err);
      }
    });
  });
