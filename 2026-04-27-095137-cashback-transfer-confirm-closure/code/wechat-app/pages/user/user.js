...
Page({
  ...
  refreshUserProfile() {
    return request({
      ...
      let mergedNickname = '';
      if (!backendNickname) {
        mergedNickname = currentNickname || '寰俊鐢ㄦ埛';
      } else if (
  ...
  isDefaultWechatNickname(nickname) {
    ...
    return /^寰俊鐢ㄦ埛[\w-]{0,16}$/i.test(value) || value === '寰俊鐢ㄦ埛';
  },
  ...
  goAddress() {
    if (!this.requireLogin('璇峰厛鐧诲綍鍚庣鐞嗗湴鍧€')) {
      return;
    }
  ...
  goOrderList() {
    if (!this.requireLogin('璇峰厛鐧诲綍鍚庢煡鐪嬭鍗?)) {
      return;
    }
  ...
  goInvite() {
    if (!this.requireLogin('璇峰厛鐧诲綍鍚庢煡鐪嬮個璇锋捣鎶?)) {
      return;
    }
  ...
  goInviteRecord() {
    if (!this.requireLogin('璇峰厛鐧诲綍鍚庢煡鐪嬮個璇疯褰?)) {
      return;
    }
  ...
  goCashback() {
    if (!this.requireLogin('璇峰厛鐧诲綍鍚庢煡鐪嬫敹鐩?)) {
      return;
    }
