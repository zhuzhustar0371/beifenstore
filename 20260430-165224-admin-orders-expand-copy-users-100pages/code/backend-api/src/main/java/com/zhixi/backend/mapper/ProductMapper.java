...
public interface ProductMapper {
  String PRODUCT_COLUMNS = "id,name,price,description,detail_content AS detailContent,main_image_url AS imageUrl,COALESCE(sales_count, 0) AS salesCount,(status = 1) AS active,(COALESCE(is_featured, 0) = 1) AS featured,created_at AS createdAt";
  ...
  @Insert("INSERT INTO products(name, sub_title, description, detail_content, main_image_url, price, status, is_featured, sort_no, created_at, updated_at) " +
      "VALUES(#{name}, #{name}, #{description}, #{detailContent}, #{imageUrl}, #{price}, CASE WHEN #{active} THEN 1 ELSE 0 END, CASE WHEN #{featured} THEN 1 ELSE 0 END, 0, NOW(), NOW())")
  @Options(useGeneratedKeys = true, keyProperty = "id")
  int insert(Product product);

  @Update("UPDATE products SET name = #{name}, sub_title = #{name}, price = #{price}, description = #{description}, detail_content = #{detailContent}, main_image_url = #{imageUrl}, " +
      "status = CASE WHEN #{active} THEN 1 ELSE 0 END, is_featured = CASE WHEN #{featured} THEN 1 ELSE 0 END, updated_at = NOW() WHERE id = #{id}")
