<template>
  <div
    :class="[
      'admin-shell relative min-h-screen overflow-hidden bg-theme-bg-1 text-theme-text-2 transition-colors duration-300',
      { 'admin-shell--mobile': isMobileAdmin, 'bg-spatial-mesh': isDark }
    ]"
  >
    <canvas
      v-show="!isMobileAdmin && isDark"
      ref="trailCanvas"
      class="pointer-events-none fixed inset-0 z-0 h-full w-full"
    />
    <div v-show="isDark" class="pointer-events-none fixed inset-0 z-0 bg-grain opacity-18 mix-blend-soft-light" />

    <div v-show="isDark" class="pointer-events-none fixed -left-28 top-24 z-0 h-80 w-80 rounded-full bg-cyan-400/18 blur-3xl animate-slow-float" />
    <div v-show="isDark" class="pointer-events-none fixed right-[-7rem] top-[18%] z-0 h-96 w-96 rounded-full bg-violet-500/18 blur-3xl animate-slow-float" />
    <div v-show="isDark" class="pointer-events-none fixed bottom-[-6rem] left-[28%] z-0 h-72 w-72 rounded-full bg-blue-500/18 blur-3xl animate-slow-float" />
    ...
    >
      ...
      <div class="hidden rounded-[24px] border border-white/20 bg-white/[0.10] p-phi-4 text-[0.75rem] font-bold text-indigo-100/68 backdrop-blur-[40px] lg:block">
        <p class="text-theme-text-1">空间界面</p>
      ...
      <header :class="['sticky top-0 z-20 py-phi-3', isMobileAdmin ? 'px-3' : 'px-phi-4 md:px-phi-5']">
        <div class="glass-panel flex items-center justify-between gap-phi-4 px-phi-4 py-phi-3">
          ...
          <div class="flex items-center gap-phi-2">
            ...
            <button class="icon-button" :aria-label="isDark ? '切换浅色模式' : '切换深色模式'" @click="toggleTheme">
...
import { ClipboardList, Gift, LayoutDashboard, LogOut, Menu, Moon, Package, RefreshCw, ShoppingCart, Sparkles, Sun, Users, WalletCards, X } from "lucide-vue-next";
import GlowIcon from "../components/GlowIcon.vue";
import RefundRequestToast from "../components/RefundRequestToast.vue";
...
const menuItems = [
  ...
  { to: "/audit-logs", label: "操作日志", icon: ClipboardList, tone: "slate" }
];
