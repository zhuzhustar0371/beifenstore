...
import com.zhixi.backend.dto.WechatMiniappPrecheckRequest;
...
public class UserAuthService {
  ...
  public byte[] getMiniappQrLoginImage(String scene) {
    WechatQrSession session = wechatQrSessionMapper.findByScene(scene);
    if (session == null) {
      throw new BusinessException("微信小程序登录会话不存在");
    }
    if (session.getExpiresAt().isBefore(LocalDateTime.now())) {
      throw new BusinessException("微信小程序登录会话已过期");
    }
  ...
  public Map<String, Object> loginByMiniapp(WechatMiniappLoginRequest request) {
    if (request.getCode() == null || request.getCode().trim().isEmpty()) {
      throw new BusinessException("缺少小程序登录code");
    }
    ...
    String inviteCode = normalizeText(request.getInviteCode(), "");
    Long fallbackInviterId = inviteCode.isBlank() ? request.getInviterId() : null;
    User user = resolveMiniappUser(identity, nickname, avatarUrl, fallbackInviterId);
    boolean inviteCodeInvalid = applyMiniappInviteCodeIfValid(user, inviteCode);
    if (!inviteCodeInvalid && !inviteCode.isBlank()) {
      user = userService.getUser(user.getId());
    }
    userService.ensureEnabled(user);
    ...
    data.put("inviteCodeInvalid", inviteCodeInvalid);
    return data;
  }

  public Map<String, Object> precheckMiniappLogin(WechatMiniappPrecheckRequest request) {
    if (request == null || request.getCode() == null || request.getCode().trim().isEmpty()) {
      throw new BusinessException("缺少小程序登录code");
    }

    WechatIdentity identity = exchangeMiniappIdentity(request.getCode());
    User existingUser = findExistingMiniappUser(identity);

    Map<String, Object> data = new HashMap<>();
    data.put("registered", existingUser != null);
    data.put("hasInviter", existingUser != null && existingUser.getInviterId() != null);
    data.put("canInputInviteCode", existingUser == null);
    return data;
  }
  ...
  public Map<String, Object> confirmMiniappQrLogin(WechatMiniappQrConfirmRequest request) {
    ...
    if (!"PENDING".equals(session.getStatus()) && !"SCANNED".equals(session.getStatus())) {
      throw new BusinessException("微信小程序登录会话不可用");
    }
  ...
  public Map<String, Object> handleWechatWebCallback(String code, String scene) {
    if (code == null || code.isBlank()) {
      throw new BusinessException("缺少微信OAuth code");
    }
    if (scene == null || scene.isBlank()) {
      throw new BusinessException("缺少微信登录scene");
    }
  ...
  public User getUserByToken(String token) {
    ...
    if (user.getStatus() == null || user.getStatus() != 1) {
      ...
      throw new BusinessException("璐﹀彿宸茶绂佺敤");
    }
    return user;
  }
  ...
  private boolean isFirstMiniappLogin(WechatIdentity identity) {
    return findExistingMiniappUser(identity) == null;
  }
  ...
  private User findExistingMiniappUser(WechatIdentity identity) {
    if (identity == null) {
      return null;
    }
    ...
    user = resolveExistingUser(userWechatAuthMapper.findBySourceAndOpenid(WECHAT_SOURCE_MINIAPP, identity.openid()));
    if (user != null) {
      return user;
    }

    user = resolveExistingUser(userWechatAuthMapper.findByOpenid(identity.openid()));
    if (user != null) {
      return user;
    }
    ...
    if (!unionid.isBlank()) {
      user = resolveExistingUser(userWechatAuthMapper.findByUnionid(unionid));
      if (user != null) {
        return user;
      }
    }
  ...
  private User resolveExistingUser(UserWechatAuth auth) {
    if (auth == null || auth.getUserId() == null) {
      return null;
    }

    try {
      return userService.getUser(auth.getUserId());
    } catch (BusinessException ex) {
      return null;
    }
  }
  ...
  private User resolveMiniappUser(
    ...
    return applyMiniappInviterIfValid(user, inviterId);
  }

  private User applyMiniappInviterIfValid(User user, Long inviterId) {
    if (user == null || inviterId == null || user.getInviterId() != null) {
      return user;
    }
    if (user.getId() != null && user.getId().equals(inviterId)) {
      log.warn(
          "Skip miniapp inviter binding because inviterId points to the same user, userId={}, inviterId={}",
          user.getId(),
          inviterId
      );
      return user;
    ...
    try {
      ...
      String inviteCode = normalizeText(inviter.getInviteCode(), "");
      if (inviteCode.isBlank()) {
        log.warn(
            "Miniapp login ignored inviter without invite code, userId={}, inviterId={}",
            user.getId(),
            inviterId
        );
        return user;
      ...
      userService.bindInviterIfNeeded(user.getId(), inviteCode);
  ...
  private boolean applyMiniappInviteCodeIfValid(User user, String inviteCode) {
    if (user == null || inviteCode == null || inviteCode.isBlank() || user.getInviterId() != null) {
      return false;
    }
    ...
    try {
      ...
      log.warn(
          "Miniapp login ignored invalid invite code, userId={}, message={}",
          user.getId(),
          ex.getMessage()
      );
      return true;
  ...
  private WechatIdentity exchangeMiniappIdentity(String code) {
    if (wechatMiniappAppId == null || wechatMiniappAppId.isBlank()) {
      throw new BusinessException("WECHAT_MINIAPP_APP_ID未配置");
    }
    if (wechatMiniappAppSecret == null || wechatMiniappAppSecret.isBlank()) {
      throw new BusinessException("WECHAT_MINIAPP_APP_SECRET未配置");
    }
  ...
  private byte[] requestMiniappWebLoginCode(String scene) {
    try {
      ...
      if (response.statusCode() < 200 || response.statusCode() >= 300) {
        throw new BusinessException("小程序二维码请求失败：HTTP " + response.statusCode());
      }
      ...
      if (looksLikeJson(payload)) {
        ...
        throw new BusinessException("小程序二维码请求失败："
  ...
  private WechatWebProfile exchangeWechatWebProfile(String code) {
    ...
    try {
      ...
      if (accessToken.isBlank() || openid.isBlank()) {
        throw new BusinessException("微信网页登录未返回access_token或openid");
      }
      ...
      throw new BusinessException("微信网页登录失败");
    }
  }
  ...
  private void validateWechatWebConfig() {
    if (wechatWebAppId == null || wechatWebAppId.isBlank()) {
      throw new BusinessException("WECHAT_WEB_APP_ID未配置");
    }
    if (wechatWebAppSecret == null || wechatWebAppSecret.isBlank()) {
      throw new BusinessException("WECHAT_WEB_APP_SECRET未配置");
    }
    if (wechatRedirectUri == null || wechatRedirectUri.isBlank()) {
      throw new BusinessException("WECHAT_WEB_REDIRECT_URI未配置");
    }
  }
