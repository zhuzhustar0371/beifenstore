  ...
  private String fetchStableAccessToken(boolean forceRefresh) {
    ...
    try {
      ...
      if (response.statusCode() < 200 || response.statusCode() >= 300) {
        throw new BusinessException("获取小程序access_token失败：HTTP " + response.statusCode());
      }
      ...
      if (errCode != 0) {
        throw new BusinessException(resolveWechatError(payload, "获取小程序access_token失败"));
      }
      ...
      throw new BusinessException("获取小程序access_token失败");
    }
  }
