...
public class DatabaseMigrationRunner implements CommandLineRunner {
  ...
  public void run(String... args) {
    ...
    ensureColumn(
        "orders.refund_complete_time",
        "SELECT refund_complete_time FROM orders LIMIT 1",
        "ALTER TABLE orders ADD COLUMN refund_complete_time TIMESTAMP NULL AFTER refund_apply_time"
    );
    ensureColumn(
    ...
    ensureColumn(
        "orders.cancel_time",
        "SELECT cancel_time FROM orders LIMIT 1",
        "ALTER TABLE orders ADD COLUMN cancel_time TIMESTAMP NULL"
    );
    ensureRenamedColumn(
    ...
    ensureAdminAccount("zhixi123123", "19ce6252cd8a8c961cf876ae4833b04dd12cf92a33794ff984ae7b5885f60452", "管理员2");
  }
  ...
  private void ensureAdminAccount(String username, String passwordHash, String displayName) {
    try {
      Integer count = jdbcTemplate.queryForObject(
          "SELECT COUNT(*) FROM admins WHERE username = ?", Integer.class, username
      );
      if (count != null && count > 0) {
        log.info("Admin account '{}' already exists, skipping.", username);
        return;
      }
      jdbcTemplate.update(
          "INSERT INTO admins(username, password_hash, display_name, status, created_at, updated_at) VALUES(?, ?, ?, 1, NOW(), NOW())",
          username, passwordHash, displayName
      );
      log.info("Created admin account '{}'.", username);
    } catch (Exception ex) {
      log.warn("Failed to ensure admin account '{}'.", username, ex);
    }
  }
