...
public interface CashbackRecordMapper {
  @Insert("INSERT INTO cashback_records(user_id,related_order_id,cashback_type,amount,related_invite_batch_id,status,remark,out_batch_no,out_detail_no,eligible_at,early_withdrawal,created_at) " +
      "VALUES(#{userId},#{orderId},#{type},#{amount},#{batchNo},#{status},#{remark},#{outBatchNo},#{outDetailNo},#{eligibleAt},COALESCE(#{earlyWithdrawal},0),NOW())")
  @Options(useGeneratedKeys = true, keyProperty = "id")
  int insert(CashbackRecord record);

  @Select("SELECT id,user_id AS userId,related_order_id AS orderId,cashback_type AS type,amount,related_invite_batch_id AS batchNo,status,remark,out_batch_no AS outBatchNo,out_detail_no AS outDetailNo,transfer_id AS transferId,transfer_detail_id AS transferDetailId,transfer_fail_reason AS transferFailReason,transfer_package_info AS transferPackageInfo,transfer_time AS transferTime,eligible_at AS eligibleAt,withdrawal_request_id AS withdrawalRequestId,early_withdrawal AS earlyWithdrawal,created_at AS createdAt " +
      "FROM cashback_records WHERE id = #{id}")
  CashbackRecord findById(Long id);

  @Select("SELECT id,user_id AS userId,related_order_id AS orderId,cashback_type AS type,amount,related_invite_batch_id AS batchNo,status,remark,out_batch_no AS outBatchNo,out_detail_no AS outDetailNo,transfer_id AS transferId,transfer_detail_id AS transferDetailId,transfer_fail_reason AS transferFailReason,transfer_package_info AS transferPackageInfo,transfer_time AS transferTime,eligible_at AS eligibleAt,withdrawal_request_id AS withdrawalRequestId,early_withdrawal AS earlyWithdrawal,created_at AS createdAt " +
      "FROM cashback_records WHERE out_detail_no = #{outDetailNo} ORDER BY id DESC LIMIT 1")
  ...
  @Select({
      "<script>",
      "SELECT id,user_id AS userId,related_order_id AS orderId,cashback_type AS type,amount,related_invite_batch_id AS batchNo,status,remark,out_batch_no AS outBatchNo,out_detail_no AS outDetailNo,transfer_id AS transferId,transfer_detail_id AS transferDetailId,transfer_fail_reason AS transferFailReason,transfer_package_info AS transferPackageInfo,transfer_time AS transferTime,eligible_at AS eligibleAt,withdrawal_request_id AS withdrawalRequestId,early_withdrawal AS earlyWithdrawal,created_at AS createdAt FROM cashback_records",
      "WHERE 1=1",
      "<if test='userId != null'>",
      "  AND user_id = #{userId}",
      "</if>",
  ...
  @Select("SELECT COUNT(1) FROM cashback_records WHERE user_id = #{userId} AND cashback_type = 'INVITE_BATCH' AND related_invite_batch_id = #{batchNo} AND status <> 'CANCELLED'")
  ...
  @Select("SELECT id,user_id AS userId,related_order_id AS orderId,cashback_type AS type,amount,related_invite_batch_id AS batchNo,status,remark,out_batch_no AS outBatchNo,out_detail_no AS outDetailNo,transfer_id AS transferId,transfer_detail_id AS transferDetailId,transfer_fail_reason AS transferFailReason,transfer_package_info AS transferPackageInfo,transfer_time AS transferTime,eligible_at AS eligibleAt,withdrawal_request_id AS withdrawalRequestId,early_withdrawal AS earlyWithdrawal,created_at AS createdAt " +
      "FROM cashback_records WHERE user_id = #{userId} AND status = 'PENDING' AND withdrawal_request_id IS NULL ORDER BY COALESCE(eligible_at, created_at) ASC, id ASC")
  List<CashbackRecord> findPendingUnrequestedByUserId(Long userId);

  @Select("SELECT id,user_id AS userId,related_order_id AS orderId,cashback_type AS type,amount,related_invite_batch_id AS batchNo,status,remark,out_batch_no AS outBatchNo,out_detail_no AS outDetailNo,transfer_id AS transferId,transfer_detail_id AS transferDetailId,transfer_fail_reason AS transferFailReason,transfer_package_info AS transferPackageInfo,transfer_time AS transferTime,eligible_at AS eligibleAt,withdrawal_request_id AS withdrawalRequestId,early_withdrawal AS earlyWithdrawal,created_at AS createdAt " +
      "FROM cashback_records WHERE user_id = #{userId} AND status = 'PENDING' AND withdrawal_request_id IS NULL AND (eligible_at IS NULL OR eligible_at <= NOW()) ORDER BY id ASC")
