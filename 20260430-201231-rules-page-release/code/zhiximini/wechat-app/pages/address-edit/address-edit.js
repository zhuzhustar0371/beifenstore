...
Page({
  data: {
    ...
    originalOrderAddressFingerprint: '',
    form: {
      name: '',
      phone: '',
      regionText: '',
  ...
  updatePageTitle() {
    const { scene, addressId, hasPreferredAddress, showAddressForm } = this.data;
    let title = '编辑地址';
    if (scene === SCENE_ORDER) {
      title = hasPreferredAddress && !showAddressForm ? '确认收货地址' : '填写收货地址';
    } else if (!addressId) {
      title = '新增地址';
    }
  ...
  onRegionChange(e) {
    ...
    this.syncRemoteRegionState(regionParts, true);
  },
    ...
    request({
      ...
      .then((res) => {
        if (res.data) {
          ...
          if (this.data.scene === SCENE_ORDER) {
            this.setData({
              hasPreferredAddress: true,
              showAddressForm: true,
              originalOrderAddressFingerprint: this.buildAddressFingerprint({
    ...
    request({
      ...
      .then((res) => {
        if (res.data) {
          ...
          this.syncRemoteRegionState([res.data.province, res.data.city, res.data.district], true);
          return;
        }
        ...
        this.syncRemoteRegionState([], false);
      })
      .catch(() => {
        this.setData({
          hasPreferredAddress: false,
  ...
  applyAddressToForm(address) {
    ...
    this.syncRemoteRegionState(hasRegion ? regionParts : [], false);
  },
  ...
  save() {
    ...
    const action = this.shouldPersistAddress(payload)
      ? this.persistAddress(payload).then((savedAddress) => {
        if (this.data.scene === SCENE_ORDER && savedAddress) {
          this.setData({
            hasPreferredAddress: true,
            originalOrderAddressFingerprint: this.buildAddressFingerprint({
  ...
  shouldPersistAddress(payload) {
    if (this.data.scene === SCENE_MANAGE) {
      return true;
    }

    if (!this.data.hasPreferredAddress || !this.data.addressId) {
      return true;
    }

    return this.buildAddressFingerprint(payload) !== this.data.originalOrderAddressFingerprint;
  },

  buildAddressFingerprint(payload) {
    const normalized = payload || {};
    return [
      this.trim(normalized.recipientName),
      this.trim(normalized.recipientPhone),
      this.trim(normalized.province),
      this.trim(normalized.city),
      this.trim(normalized.district),
      this.trim(normalized.detailAddress),
  ...
  syncRemoteRegionState(regionParts, showToast) {
    const province = Array.isArray(regionParts) && regionParts.length > 0 ? this.trim(regionParts[0]) : '';
    const remoteRegionName = this.resolveRemoteRegionName(province);
    const isRemoteRegion = !!remoteRegionName;

    this.setData({
      isRemoteRegion,
      remoteRegionName,
      shippingFeeText: isRemoteRegion ? REMOTE_SHIPPING_FEE.toFixed(2) : '0.00'
    });

    if (isRemoteRegion && showToast) {
      wx.showToast({
        title: `当前地区运费+${REMOTE_SHIPPING_FEE}元`,
        icon: 'none'
      });
    }
  },

  resolveRemoteRegionName(province) {
    const value = this.trim(province);
    if (!value) {
      return '';
    }
    const matched = REMOTE_REGIONS.find((item) => item === value);
    return matched || '';
  },
