...
public interface WithdrawalRequestMapper {
  @Insert("INSERT INTO withdrawal_requests(user_id,amount,status,source,remark,idempotency_key,request_no,created_at) " +
      "VALUES(#{userId},#{amount},#{status},#{source},#{remark},#{idempotencyKey},#{requestNo},NOW())")
  ...
  @Select("SELECT id,user_id AS userId,amount,status,source,remark,idempotency_key AS idempotencyKey,request_no AS requestNo,created_at AS createdAt,approved_at AS approvedAt,completed_at AS completedAt " +
  ...
  @Select({
      "<script>",
      "SELECT id,user_id AS userId,amount,status,source,remark,idempotency_key AS idempotencyKey,request_no AS requestNo,created_at AS createdAt,approved_at AS approvedAt,completed_at AS completedAt",
  ...
  @Select("SELECT COUNT(1) FROM withdrawal_requests WHERE user_id = #{userId} AND status = 'PENDING'")
