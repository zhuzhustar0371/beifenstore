<template>
  <section>
    <PageHeader
      eyebrow="Orders"
      title="订单管理"
      description="保留主表关键信息，并将发货明细折叠到展开行中，便于更紧凑地处理订单。"
        ...
        <table class="data-table orders-table">
          <thead>
            <tr>
              ...
              <th class="table-id-column">编号</th>
          ...
          <tbody>
            <template v-for="order in orders" :key="order.id">
              <tr class="order-row">
                ...
                <td class="table-id-column">#{{ order.id }}</td>
                <td class="order-no-column">
                  <div class="order-no-cell">
                    <div class="order-no-copy-group">
                      <button
                        type="button"
                        class="order-no-copy-button"
                        :title="order.orderNo || ''"
                        @click="copyOrderNo(order)"
                      >
              ...
              <tr v-if="isExpanded(order.id)" class="order-expand-row">
                <td :colspan="6" class="order-expand-cell">
                  <div class="order-expand-panel">
                    <div class="order-expand-panel__summary">
                      <div class="order-detail-block">
                        <p class="order-detail-block__label">发货信息</p>
                        <p class="order-detail-block__value">{{ recipientSummary(order) }}</p>
                        <p class="order-detail-block__meta">用户：{{ displayUserName(order) }} (#{{ order.userId || "-" }})</p>
                        <p class="order-detail-block__meta">地址：{{ addressText(order) }}</p>
                      </div>

                      <div class="order-detail-block">
                        <p class="order-detail-block__label">订单补充信息</p>
                        <p class="order-detail-block__value">{{ formatDateTime(order.createdAt) }}</p>
                        <p class="order-detail-block__meta">支付方式：{{ order.payType || "-" }}</p>
                        <p class="order-detail-block__meta">退款状态：{{ refundStatusText(order.refundStatus) }}</p>
                      </div>
                    </div>

                    <div class="order-expand-panel__shipping">
                      <div class="order-detail-block">
                        <p class="order-detail-block__label">物流单号</p>
                        <p class="order-detail-block__value">{{ trackingNoText(order) }}</p>
                        <p class="order-detail-block__meta">展开后可直接填写物流单号和快递公司，再使用主表操作列发货。</p>
                      </div>
  ...
  <div v-if="refundDialog.order" class="modal-overlay" @click.self="closeRefundDialog">
    <div class="modal-card">
      ...
      <div class="modal-body">
        ...
        <div v-else-if="refundDialog.preview" class="refund-no-cashback">该订单无关联返现记录</div>
...
async function loadOrders() {
  ...
  try {
    ...
    orders.value.forEach((order) => {
      ...
      if (!draft.expressCompany && order.expressCompany) {
        draft.expressCompany = order.expressCompany;
      }
    });
...
.order-expand-panel__summary,
.order-expand-panel__shipping {
  display: grid;
  gap: 16px;
}
