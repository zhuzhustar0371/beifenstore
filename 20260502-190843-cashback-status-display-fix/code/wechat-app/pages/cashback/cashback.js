...
function createEmptyWithdrawPreview() {
  ...
  const activeModePreview = findModePreview(modeList, 'MATURED_ONLY');
  return {
    hasPendingDebt: false,
    pendingDebtTotal: '0.00',
    pendingDebtCount: 0,
    pendingDebts: [],
    recommendedApplyMode: 'MATURED_ONLY',
    activeMode: 'MATURED_ONLY',
    activeModePreview,
    modeList
  };
}
...
Page({
  ...
  loadList() {
    ...
    Promise.all([
      ...
      const withdrawPreview = this.normalizeWithdrawPreview(summary, this.data.withdrawPreview.activeMode);
      let total = 0;
      let withdrawable = 0;
      let immature = 0;
      let requested = 0;
  ...
  normalizeWithdrawPreview(summary, currentMode) {
    ...
    const activeMode = WITHDRAW_MODE_ORDER.includes(currentMode) ? currentMode : recommendedApplyMode;
