    ...
    <div class="container hero-inner">
      <div class="hero-content">
        ...
        <div class="hero-actions">
          ...
          <RouterLink :to="buildRulesRoute(heroProduct)" class="btn btn-hero-outline btn-lg">了解返现规则</RouterLink>
        </div>
      ...
      <div class="hero-visual">
        <div class="hero-card">
          <div class="hero-card-header">
            <span class="hero-card-label">首选商品</span>
            <h3>{{ heroProduct?.name || "知禧净享洗衣液" }}</h3>
          </div>
          ...
          <div class="hero-card-summary">
            <div class="hero-card-row">
              <span>单瓶到手</span>
              <strong class="hero-card-price">¥{{ heroPriceText }}</strong>
            </div>
            <div class="hero-card-row">
              <span>适用场景</span>
            ...
            <div class="hero-card-row">
              <span>会员权益</span>
              <strong>{{ heroProduct?.featured ? "首选价参与默认返现" : "复购自动参与返现" }}</strong>
            </div>
          </div>
        ...
        <div v-for="product in products" :key="product.id" class="product-card">
          ...
          <div class="product-info">
            ...
            <div class="product-bottom">
              <span class="product-price">¥{{ product.priceText || product.price }}</span>
      ...
      <div class="cashback-grid">
        <div class="cashback-card">
          <div class="cashback-card-header">
            ...
            <div class="cashback-card-copy">
              <h3>个人复购返现</h3>
              <p>只统计当前商品的复购顺序，不与其他商品混算，支付成功后自动进入返现记录。</p>
            </div>
          </div>
          ...
          <RouterLink :to="buildRulesRoute(heroProduct)" class="cashback-more">查看完整规则 →</RouterLink>
        </div>
        ...
        <div class="cashback-card">
          <div class="cashback-card-header">
            ...
            <div class="cashback-card-copy">
              <h3>邀请返现</h3>
              <p>邀请返现按“邀请人 + 商品”独立统计，达到当前商品门槛后自动结算。</p>
            </div>
          </div>
...
const heroPriceText = computed(() => heroProduct.value?.priceText || "99");
...
function buildRulesRoute(product) {
...
async function loadCashbackRules(productId) {
  try {
    cashbackRules.value = await fetchCashbackRules(productId || undefined);
  } catch {
...
watch(
  () => heroProduct.value?.id || null,
  (productId) => {
    loadCashbackRules(productId);
  },
  { immediate: true }
);
</script>
