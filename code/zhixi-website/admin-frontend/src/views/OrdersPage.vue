<template>
  <section>
    ...
    <GlassCard>
      <div class="orders-toolbar">
        <div class="orders-toolbar__search">
          ...
          <input
            ...
            @keydown.enter.prevent="applySearch"
          />
          <select v-model="filters.status" class="table-input" @change="applySearch">
          ...
          <button class="btn-inline" :disabled="loading || (!filters.keyword && !filters.userId && !filters.status)" @click="resetSearch">
            重置筛选
          </button>
          <button class="btn-inline btn-inline--success" :disabled="loading || exporting" @click="onExportOrders">
            {{ exporting ? "导出中..." : "导出订单" }}
          </button>
          <button
      ...
      <div v-else-if="orders.length > 0 && isMobileAdmin" class="orders-mobile-list">
        <article v-for="order in orders" :key="order.id" class="orders-mobile-card">
          <div class="orders-mobile-card__top">
            <div class="min-w-0">
              <p class="orders-mobile-card__eyebrow">#{{ order.id }}</p>
              <button type="button" class="orders-mobile-card__orderno" @click="copyOrderNo(order)">
                {{ order.orderNo || "-" }}
              </button>
              <p class="orders-mobile-card__time">{{ formatDateTime(order.createdAt) }}</p>
            </div>
            <div class="orders-mobile-card__amount">楼{{ formatMoney(order.totalAmount) }}</div>
          </div>

          <div class="orders-mobile-card__badges">
            ...
            <span
              v-if="order.refundRequestStatus && order.refundRequestStatus !== 'NONE'"
              :class="['status-badge', refundRequestStatusClass(order.refundRequestStatus)]"
            >
              {{ refundRequestStatusText(order.refundRequestStatus) }}
            </span>
            <button v-if="order.orderNo" type="button" class="order-copy-chip order-copy-chip--mobile" @click="copyOrderNo(order)">
              {{ copyButtonText(order.id) }}
            </button>
          </div>

          <div class="orders-mobile-card__section">
            <p class="orders-mobile-card__label">{{ displayUserName(order) }}</p>
            <p class="orders-mobile-card__value">ID {{ order.userId || "-" }}</p>
            <p class="orders-mobile-card__meta">{{ recipientSummary(order) }}</p>
            <p class="orders-mobile-card__meta">{{ addressText(order) }}</p>
          </div>

          <div class="orders-mobile-card__section">
            <p class="orders-mobile-card__label">Payment</p>
            <p class="orders-mobile-card__meta">{{ paymentKeyText(order) }}</p>
          </div>

          <button
            type="button"
            class="orders-mobile-card__toggle"
            :aria-expanded="isExpanded(order.id)"
            @click="toggleExpanded(order.id)"
          >
            {{ isExpanded(order.id) ? "收起详情" : "展开详情" }}
          </button>

          <div v-if="isExpanded(order.id)" class="orders-mobile-card__details">
            <div class="order-detail-card">
              <p class="order-detail-card__label">商品信息</p>
              <div class="product-card">
                <img
                  v-if="order.productImageUrl"
                  :src="resolveImageUrl(order.productImageUrl)"
                  :alt="order.productName || '商品图片'"
                  class="product-card__image"
                  referrerpolicy="no-referrer"
                />
                <div v-else class="product-card__image product-card__image--fallback">图</div>
                <div class="product-card__info">
                  <p class="order-detail-card__value">{{ order.productName || "-" }}</p>
                  <p class="order-detail-card__meta">单价：¥{{ formatMoney(order.productAmount) }} &times; {{ order.quantity || 1 }} 件</p>
                  <p class="order-detail-card__meta">实付：¥{{ formatMoney(order.totalAmount) }}</p>
                </div>
              </div>
            </div>
            <div class="order-detail-card">
              ...
              <div class="shipping-fields shipping-fields--mobile">
                <input
                ...
                <input
                  ...
                  class="table-input"
                  placeholder="快递公司编码，如 SF"
                />
              </div>
            </div>

            <div v-if="order.refundRequestStatus && order.refundRequestStatus !== 'NONE'" class="order-detail-card">
              <p class="order-detail-card__label">退款申请信息</p>
              <div class="refund-request-info">
                <div class="refund-request-info__row">
                  <span class="refund-request-info__key">申请状态</span>
                  <span :class="['status-badge', refundRequestStatusClass(order.refundRequestStatus)]">
                    {{ refundRequestStatusText(order.refundRequestStatus) }}
                  </span>
                </div>
                <div v-if="order.refundRequestReason" class="refund-request-info__row">
                  <span class="refund-request-info__key">退款理由</span>
                  <span class="refund-request-info__value">{{ order.refundRequestReason }}</span>
                </div>
                <div v-if="order.refundRequestAt" class="refund-request-info__row">
                  <span class="refund-request-info__key">申请时间</span>
                  <span class="refund-request-info__value">{{ formatDateTime(order.refundRequestAt) }}</span>
                </div>
                <div v-if="order.status === 'COMPLETED'" class="refund-request-info__row">
                  <span class="refund-request-info__key">订单状态</span>
                  <span class="refund-request-info__value" style="color: #fbbf24; font-weight: 800;">已确认收货（退款需特别注意）</span>
                </div>
                <div v-if="order.refundReviewRemark" class="refund-request-info__row">
                  <span class="refund-request-info__key">审批备注</span>
                  <span class="refund-request-info__value">{{ order.refundReviewRemark }}</span>
                </div>
                <div v-if="order.refundCompletedAt" class="refund-request-info__row">
                  <span class="refund-request-info__key">退款完成时间</span>
                  <span class="refund-request-info__value">{{ formatDateTime(order.refundCompletedAt) }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="orders-mobile-card__actions">
            <button
              v-if="canCancelShipment(order)"
               class="btn-inline btn-inline--danger"
              :disabled="cancelShipmentId === order.id"
              @click="onCancelShipment(order)"
            >
              {{ cancelShipmentId === order.id ? "取消中..." : "取消发货" }}
            </button>
            <button v-else class="btn-inline btn-inline--success" :disabled="shippingId === order.id || !canShip(order)" @click="onShip(order)">
              {{ shippingId === order.id ? "发货中..." : (canShip(order) ? "发货" : "不可发货") }}
            </button>
            <template v-if="order.refundRequestStatus === 'PENDING'">
              <button class="btn-inline btn-inline--accent" :disabled="approvingId === order.id" @click="openApproveDialog(order)">
                {{ approvingId === order.id ? "处理中..." : "批准退款" }}
              </button>
              <button class="btn-inline btn-inline--danger" :disabled="rejectingId === order.id" @click="onReject(order)">
                {{ rejectingId === order.id ? "处理中..." : "驳回申请" }}
              </button>
            </template>
            <button v-else class="btn-inline btn-inline--warning" :disabled="refundingId === order.id || !canRefund(order)" @click="openRefundDialog(order)">
              {{ refundingId === order.id ? "处理中..." : (canRefund(order) ? "退款" : "不可退款") }}
            </button>
          </div>
        </article>

        <div class="orders-pagination orders-pagination--mobile">
                ...
                <td :colspan="5" class="order-expand-cell">
                  <div class="order-expand-panel">
                    <div class="order-expand-grid">
                      ...
                      <section v-if="order.refundRequestStatus && order.refundRequestStatus !== 'NONE'" class="order-detail-card order-detail-card--wide">
                        <p class="order-detail-card__label">退款申请信息</p>
                        <div class="refund-request-info">
                          <div class="refund-request-info__row">
                            <span class="refund-request-info__key">申请状态</span>
  ...
  <div v-if="refundDialog.order" class="modal-overlay" @click.self="closeRefundDialog">
    <div class="modal-card">
      ...
      <div class="modal-body">
        ...
        <div v-if="refundDialog.order.refundCompletedAt" class="refund-info-row">
          <span>退款完成时间</span>
          <span class="text-white/80">{{ formatDateTime(refundDialog.order.refundCompletedAt) }}</span>
        </div>
        ...
        <div v-if="refundDialog.userReason" class="refund-user-reason-section">
          <label class="refund-amount-label">用户退款申请理由</label>
          <p class="refund-user-reason-text">{{ refundDialog.userReason }}</p>
        </div>

        <div class="refund-reason-section">
          <label class="refund-amount-label">{{ refundDialog.userReason ? "管理员备注" : "退款原因" }}</label>
          <input v-model.trim="refundDialog.reason" class="refund-amount-input" :placeholder="refundDialog.userReason ? '管理员审批备注' : '协商退款'" />
        </div>
      </div>
...
const cancelShipmentId = ref(null);
...
function syncRouteQuery() {
  ...
  if (nextUserId) {
    nextQuery.userId = String(nextUserId);
  }
  ...
  delete currentQuery._t;
  const keys = new Set([...Object.keys(currentQuery), ...Object.keys(nextQuery)]);
  let changed = false;
  for (const key of keys) {
    const current = currentQuery[key] ? String(currentQuery[key]) : "";
...
function canCancelShipment(order) {
  return !!order && order.status === "SHIPPED" && order.refundStatus === "NONE";
}
...
async function onCancelShipment(order) {
  if (!canCancelShipment(order)) return;

  const ok = window.confirm(
    `确认取消订单 #${order.id} 的发货？\n\n` +
    `物流单号：${order.trackingNo || "-"}\n` +
    `此操作将撤销发货状态，订单将回到「已支付」状态。\n` +
    `注意：微信平台的发货信息无法同步撤销，如需重新发货将视为幂等操作。`
  );
  if (!ok) return;

  cancelShipmentId.value = order.id;
  try {
    ...
    const message = error?.response?.data?.message || error?.message || "取消发货失败";
    window.alert(message);
  } finally {
    cancelShipmentId.value = null;
  }
}
...
function refundRequestStatusClass(status) {
  if (status === "PENDING") return "warning";
...
function refundRequestStatusText(status) {
  const map = {
    NONE: "",
    PENDING: "待审批退款",
    APPROVED: "已批准退款",
...
async function openRefundDialog(order) {
  ...
  refundDialog.userReason = order.refundRequestReason && order.refundRequestStatus === "PENDING" ? order.refundRequestReason : "";
  refundDialog.reason = refundDialog.userReason ? "" : "协商退款";
...
async function openApproveDialog(order) {
...
async function confirmRefund() {
  ...
  const isApproval = order.refundRequestStatus === "PENDING";
  ...
  try {
    if (isApproval) {
      await approveRefundRequest(order.id, amount, reason);
    } else {
      await requestRefundOrder(order.id, reason, amount);
    }
    closeRefundDialog();
    await loadOrders();
    window.alert(`订单 #${order.id} 退款${isApproval ? "批准" : "提交"}成功，退款金额：¥${formatMoney(amount)}`);
  } catch (error) {
...
async function onReject(order) {
  const remark = window.prompt("请输入驳回备注（选填）：", "");
  if (remark === null) return;

  rejectingId.value = order.id;
  try {
    await rejectRefundRequest(order.id, remark.trim() || undefined);
    await loadOrders();
    window.alert(`订单 #${order.id} 退款申请已驳回`);
  } catch (error) {
    const message = error?.response?.data?.message || error?.message || "驳回失败";
    window.alert(message);
  } finally {
...
watch(
  ...
  (value) => {
    const raw = Array.isArray(value) ? value[0] : value;
    const normalized = raw && String(raw).trim() ? String(raw).trim() : "";
    if (filters.keyword !== normalized) {
...
onMounted(() => {
  ...
  if (rawUserId && Number.isInteger(Number(rawUserId)) && Number(rawUserId) > 0) {
    filters.userId = String(Number(rawUserId));
  }
  const rawKeyword = Array.isArray(route.query.keyword) ? route.query.keyword[0] : route.query.keyword;
  if (rawKeyword && String(rawKeyword).trim()) {
    filters.keyword = String(rawKeyword).trim();
  }
  loadOrders();
});
...
.expand-toggle {
  ...
  border-radius: 999px;
  background: linear-gradient(135deg, rgba(255, 255, 255, 0.20), rgba(255, 255, 255, 0.06) 50%, rgba(56, 189, 248, 0.10));
...
.expand-toggle:hover {
  background: linear-gradient(135deg, rgba(255, 255, 255, 0.30), rgba(255, 255, 255, 0.12) 50%, rgba(56, 189, 248, 0.22));
  border-color: rgba(255, 255, 255, 0.55);
  box-shadow: 0 6px 32px rgba(56, 189, 248, 0.24),
...
.shipping-fields--mobile {
  margin-top: 14px;
}

.order-copy-chip--mobile {
  opacity: 1;
  transform: none;
}
...
/* Refund request info in expanded panel */
.refund-request-info {
  display: flex;
  flex-direction: column;
  gap: 10px;
...
.refund-request-info__value {
  color: rgba(255, 255, 255, 0.82);
  word-break: break-word;
}

/* === 浅色模式对比度修复 === */
:root:not(.dark) .orders-toolbar__meta {
  color: rgba(71, 85, 105, 0.9);
}

:root:not(.dark) .payment-key-line {
  color: rgba(71, 85, 105, 0.75);
}

:root:not(.dark) .expand-toggle {
...
:root:not(.dark) .expand-toggle:hover {
...
:root:not(.dark) .orders-mobile-card {
  background: rgba(241, 245, 249, 0.85);
  border-color: rgba(148, 163, 184, 0.35);
}

:root:not(.dark) .orders-mobile-card__orderno {
  color: #0f172a;
}

:root:not(.dark) .orders-mobile-card__eyebrow {
  color: #0e7490;
}

:root:not(.dark) .orders-mobile-card__time,
:root:not(.dark) .orders-mobile-card__meta {
  color: rgba(71, 85, 105, 0.9);
}

:root:not(.dark) .orders-mobile-card__label {
  color: #0f172a;
}

:root:not(.dark) .orders-mobile-card__amount {
  color: #0891b2;
}

:root:not(.dark) .orders-mobile-card__value {
  color: rgba(71, 85, 105, 0.9);
}

:root:not(.dark) .orders-mobile-card .order-detail-card {
  background: rgba(255, 255, 255, 0.6);
  border-color: rgba(148, 163, 184, 0.25);
}

:root:not(.dark) .orders-mobile-card .order-detail-card__label {
  color: #0e7490;
}

:root:not(.dark) .orders-mobile-card .order-detail-card__value {
  color: #0f172a;
}

:root:not(.dark) .orders-mobile-card .order-detail-card__meta {
  color: rgba(71, 85, 105, 0.9);
}
</style>
