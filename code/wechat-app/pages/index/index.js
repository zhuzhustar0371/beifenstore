...
const RULE_ITEMS = [
  ...
  '邀请返现按“邀请人 + 商品”独立统计',
  '不同商品的返现阶梯互不串单',
  '发生退款退货时，相关返现会按当前商品规则重算',
  '禁止刷单、自买、虚假交易，违规将取消奖励'
];
...
Page({
    ...
    request({
      ...
      .then((res) => {
        ...
        if (products.length > STREAM_THRESHOLD) {
          ...
          diffSetData(this, nextData, () => this.streamRemainingProducts(products, STREAM_THRESHOLD));
          return;
        }

        nextData.visibleProducts = products;
        diffSetData(this, nextData);
      })
      .catch(() => {
        this.clearStreamTimer();
        diffSetData(this, {
  ...
  streamRemainingProducts(products, startIndex) {
    ...
    const endIndex = Math.min(startIndex + STREAM_BATCH_SIZE, products.length);
