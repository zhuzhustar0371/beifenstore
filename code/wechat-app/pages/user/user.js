...
Page({
  ...
  fetchUserStats() {
    ...
    Promise.all([
      request({ url: '/api/cashbacks/me/summary', method: 'GET' }).catch(() => ({ data: null })),
      request({ url: `/api/invites/${userId}`, method: 'GET' }).catch(() => ({ data: [] }))
    ]).then(([summaryRes, invitesRes]) => {
      const summary = summaryRes.data || {};
      const invites = invitesRes.data || [];

      const settlingTotal = Number(summary.settlingTotal || 0);
      const transferredTotal = Number(summary.transferredTotal || 0);

      diffSetData(this, {
        stats: {
          balance: (Number.isFinite(settlingTotal) ? settlingTotal : 0).toFixed(2),
          totalEarned: (Number.isFinite(transferredTotal) ? transferredTotal : 0).toFixed(2),
  ...
  logout() {
    ...
    diffSetData(this, { hasLogin: false, userInfo: null });
    this.resetStats();
  },
