...
function isInviteBatchCashback(item) {
  return !!item && (item.type === 'INVITE_BATCH' || item.cashbackType === 'INVITE_BATCH');
}

function isWithdrawableCashback(item) {
  if (!item || (item.status !== 'PENDING' && item.status !== 'SETTLED')) {
    return false;
  }
  if (item.withdrawalRequestId) {
    return false;
  }
  return isInviteBatchCashback(item) || isEligibleTimeReached(item.eligibleAt);
}
  ...
  fetchUserStats() {
    ...
    Promise.all([
      ...
      cashbacks.forEach((item) => {
        ...
        if (isWithdrawableCashback(item)) {
