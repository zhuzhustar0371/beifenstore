...
public interface CashbackRecordMapper {
  @Insert("INSERT INTO cashback_records(user_id,related_order_id,cashback_type,amount,related_invite_batch_id,status,remark,out_batch_no,out_detail_no,eligible_at,created_at) " +
      "VALUES(#{userId},#{orderId},#{type},#{amount},#{batchNo},#{status},#{remark},#{outBatchNo},#{outDetailNo},#{eligibleAt},NOW())")
  @Options(useGeneratedKeys = true, keyProperty = "id")
  int insert(CashbackRecord record);

  @Select("SELECT id,user_id AS userId,related_order_id AS orderId,cashback_type AS type,amount,related_invite_batch_id AS batchNo,status,remark,out_batch_no AS outBatchNo,out_detail_no AS outDetailNo,transfer_id AS transferId,transfer_detail_id AS transferDetailId,transfer_fail_reason AS transferFailReason,transfer_package_info AS transferPackageInfo,transfer_time AS transferTime,eligible_at AS eligibleAt,withdrawal_request_id AS withdrawalRequestId,created_at AS createdAt " +
  ...
  @Select("SELECT id,user_id AS userId,related_order_id AS orderId,cashback_type AS type,amount,related_invite_batch_id AS batchNo,status,remark,out_batch_no AS outBatchNo,out_detail_no AS outDetailNo,transfer_id AS transferId,transfer_detail_id AS transferDetailId,transfer_fail_reason AS transferFailReason,transfer_package_info AS transferPackageInfo,transfer_time AS transferTime,eligible_at AS eligibleAt,withdrawal_request_id AS withdrawalRequestId,created_at AS createdAt " +
      "FROM cashback_records WHERE withdrawal_request_id = #{requestId} ORDER BY id ASC")
  List<CashbackRecord> findByWithdrawalRequestId(Long requestId);
  ...
  @Select({
      "<script>",
      "SELECT id,user_id AS userId,related_order_id AS orderId,cashback_type AS type,amount,related_invite_batch_id AS batchNo,status,remark,out_batch_no AS outBatchNo,out_detail_no AS outDetailNo,transfer_id AS transferId,transfer_detail_id AS transferDetailId,transfer_fail_reason AS transferFailReason,transfer_package_info AS transferPackageInfo,transfer_time AS transferTime,eligible_at AS eligibleAt,withdrawal_request_id AS withdrawalRequestId,created_at AS createdAt FROM cashback_records",
      "WHERE 1=1",
      "<if test='userId != null'>",
      "  AND user_id = #{userId}",
      "</if>",
  ...
  @Select("SELECT id,user_id AS userId,related_order_id AS orderId,cashback_type AS type,amount,related_invite_batch_id AS batchNo,status,remark,out_batch_no AS outBatchNo,out_detail_no AS outDetailNo,transfer_id AS transferId,transfer_detail_id AS transferDetailId,transfer_fail_reason AS transferFailReason,transfer_package_info AS transferPackageInfo,transfer_time AS transferTime,eligible_at AS eligibleAt,withdrawal_request_id AS withdrawalRequestId,created_at AS createdAt " +
      "FROM cashback_records WHERE user_id = #{userId} AND status = 'PENDING' AND withdrawal_request_id IS NULL AND (cashback_type = 'INVITE_BATCH' OR eligible_at IS NULL OR eligible_at <= NOW()) ORDER BY id ASC")
