            ...
            <tr v-for="cashback in cashbacks" :key="cashback.id">
              ...
              <td>
                <div class="action-stack">
                  <span class="text-xs text-white/45">{{ transferEligibilityText(cashback) }}</span>
                  <button
                    v-if="cashback.status === 'PENDING' || isRetryableTransferFailure(cashback)"
                    class="btn-inline"
                    :disabled="!canTransferCashback(cashback) || transferringId === cashback.id"
...
let withdrawalEventSource = null;
...
function parseEligibleTimestamp(eligibleAt) {
  if (!eligibleAt) {
    return null;
  }
...
function isTransferReady(cashback) {
  const timestamp = parseEligibleTimestamp(cashback?.eligibleAt);
  return timestamp === null || (Number.isFinite(timestamp) && timestamp <= Date.now());
}

function canTransferCashback(cashback) {
  return isTransferReady(cashback) && (cashback?.status === "PENDING" || isRetryableTransferFailure(cashback));
}

function transferEligibilityText(cashback) {
  if (!cashback?.eligibleAt) {
    return "立即可打款";
  }
  return isTransferReady(cashback) ? "已满7天" : `满7天时间：${cashback.eligibleAt}`;
}
...
async function onTransferCashback(cashback) {
  if (!canTransferCashback(cashback)) {
    showTransferMessage(`返现 #${cashback.id} 未满7天，最早可打款时间：${cashback.eligibleAt || "-"}`, "danger");
    return;
  }
  const ok = window.confirm(
...
async function onApproveWithdrawal(withdrawal) {
  ...
  try {
    const updated = await approveWithdrawalRequest(withdrawal.id);
    await Promise.all([loadWithdrawals(), loadCashbacks()]);
    showTransferMessage(`提现申请 #${withdrawal.id} 已批准，当前状态：${withdrawalStatusText(updated?.status)}。`, "success");
  } catch (error) {
...
function openWithdrawalEvents() {
  withdrawalEventSource = createWithdrawalEventSource();
  if (!withdrawalEventSource) {
    return;
  }
  withdrawalEventSource.addEventListener("withdrawal-created", () => {
    showTransferMessage("收到新的提现申请", "success");
    loadWithdrawals();
  });
  withdrawalEventSource.onerror = () => {};
}

onMounted(() => {
  loadCashbacks();
  loadWithdrawals();
  openWithdrawalEvents();
});

onBeforeUnmount(() => {
  if (withdrawalEventSource) {
    withdrawalEventSource.close();
    withdrawalEventSource = null;
  }
});
</script>
