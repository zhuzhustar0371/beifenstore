...
﻿const EnhancedMessageDetailPage = {
  setup() {
    ...
    function handleSocketPayload(payload) {
      ...
      if (payload.type === "service:message:new") {
        const message = payload.data?.message || null;
        if (!message) return;
        if (!hasMessage(message.id)) {
          upsertMessage({
            ...message,
            sender_type: "service",
            sender_name: message.sender_name || "官方客服",
          });
          scrollToBottom();
        }
        return;
  ...
  template: `
    ...
    <section v-else-if="detail" class="chat-shell">
      ...
      <div v-if="detail.service_identity" class="chat-service-banner card">
        <div class="service-banner-icon">🛡️</div>
        <div class="service-banner-info">
          <div class="service-banner-name">{{ detail.service_identity.name }}<span class="service-banner-badge">{{ detail.service_identity.badge }}</span></div>
          <div class="service-banner-desc">官方客服已介入此对话，客服消息带有专属标识</div>
        </div>
      </div>
      ...
      <section ref="panelRef" class="chat-panel card">
        <div class="chat-history">
          <div v-for="(message, index) in messages" :key="getMessageKey(message, index)" class="chat-message-block">
            <div class="chat-message-time">{{ formatDateTime(message.created_at) }}</div>
            <div class="chat-message-row" :class="{ mine: isMine(message), 'service-message': message.sender_type === 'service' }">
              <div v-if="message.sender_type === 'service'" class="chat-avatar service-avatar">🛡️</div>
              <div class="chat-avatar" v-else-if="!isMine(message)">{{ getAvatarText(peerName) }}</div>
              <div class="chat-bubble" :class="{ mine: isMine(message), 'chat-bubble-image': isImageMessage(message), 'service-bubble': message.sender_type === 'service' }">
                <div v-if="message.sender_type === 'service' && !isMine(message)" class="chat-service-label">
                  <span class="service-label-icon">🛡️</span>
                  <span class="service-label-name">{{ message.sender_name || '官方客服' }}</span>
                  <span class="service-label-badge">客服</span>
                </div>
