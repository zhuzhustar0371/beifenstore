...
const FALLBACK_CATEGORIES = [
  { id: "cat-1", name: "�ֻ�����", icon: "??", description: "�ֻ���ƽ�塢�������" },
  { id: "cat-2", name: "���԰칫", icon: "??", description: "�ʼǱ�����ʾ�����칫�豸" },
  { id: "cat-3", name: "���õ���", icon: "??", description: "С�ҵ硢�ҵ��������ⷿ����" },
  { id: "cat-4", name: "����Ь��", icon: "??", description: "�·���Ьѥ���������" },
  { id: "cat-5", name: "��ױ����", icon: "??", description: "��ױ����������˻���" },
  { id: "cat-6", name: "ĸӤ��Ʒ", icon: "??", description: "ĸӤ����ͯ�ɳ���Ʒ" },
  { id: "cat-7", name: "�Ҿ�����", icon: "??", description: "���ðٻ���������ࡢ�Ҿ��ӻ�" },
  { id: "cat-8", name: "�˶�����", icon: "??", description: "¶Ӫ�����С��˶�����" },
  { id: "cat-9", name: "ͼ������", icon: "??", description: "ͼ�顢�������Ĵ��ܱ�" },
  { id: "cat-10", name: "��Ʒ����", icon: "??", description: "�綯�������г�����Ʒ���" },
  { id: "cat-11", name: "��������", icon: "??", description: "����������������" },
];
...
const HERO_BANNERS = [
  ...
  },
  {
    eyebrow: "��Ҳ�ܷ�",
    title: "����������������һ�׽������",
    text: "��ҳ�����ࡢ��������Ϣ�͸�������ͳһ��һ������ Web ���顣",
  },
  {
    eyebrow: "���ط����ѽ���",
    title: "ȫ�����ذ�ʡ�ݡ����С�������������ɸѡ",
    text: "��ѡʡ�У���ѡ���أ�����ͷ�������ͬһ�����ط��ࡣ",
  },
];
...
async function apiRequest(path, options = {}) {
  ...
  if (!response.ok || !payload.success) {
    throw new Error(payload.message || `����ʧ�ܣ�״̬�� ${response.status}`);
  }
  ...
  setup() {
    ...
    async function loadListings() {
      ...
      try {
        ...
        errorText.value = error.message || "��������ʧ�ܣ����Ժ����ԡ�";
      } finally {
        loading.value = false;
      }
    }
    ...
    async function openChat(item) {
      ...
      try {
        ...
        window.alert(error.message || "��������ʧ�ܣ����Ժ����ԡ�");
      }
    }

    function categoryNameOf(item) {
      return findCategory(item.category_id)?.name || "��������";
    }
  ...
  template: `
    <section class="home-hero card">
      ...
      <div class="home-hero-panel">
        <div class="hero-metric"><strong>{{ selectedScopeLabel }}</strong><span>��ǰ����</span></div>
        <div class="hero-metric"><strong>{{ listings.length }}</strong><span>���ɼ�����</span></div>
        <div class="hero-metric"><strong>{{ appState.categories.length || 11 }}</strong><span>�����÷���</span></div>
      </div>
    </section>
    ...
    <section class="card section search-section">
      <div class="row wrap search-row">
        ...
        <input class="input search-input" v-model.trim="filters.keyword" placeholder="���ֻ������������г���������..." @keydown.enter="loadListings" />
        <button class="btn btn-dark search-button" @click="loadListings">����</button>
      </div>
      <div class="district-scope-row">
        ...
        <div class="district-scope-hint">�Ѱ�ʡ / �� / �����������࣬��ǰ�� {{ districtOptions.length }} ����ѡ����</div>
      </div>
      <div class="home-category-strip">
        ...
        <router-link class="category-pill more" to="/categories">ȫ������</router-link>
      </div>
    </section>

    <section class="card section section-compact">
      <div class="segment-tabs">
        <button class="segment-tab" :class="{ active: filters.listing_type === 'all' }" @click="setListingType('all')">ȫ��</button>
        <button class="segment-tab" :class="{ active: filters.listing_type === 'sale' }" @click="setListingType('sale')">����</button>
        <button class="segment-tab" :class="{ active: filters.listing_type === 'wanted' }" @click="setListingType('wanted')">��</button>
      </div>
      <div class="section-caption">��ǰ������{{ selectedScopeLabel }} <span>�� {{ listings.length }} �����</span></div>
    </section>

    <section v-if="loading" class="card section muted">���ڼ�������...</section>
    ...
    <section v-else class="listing-grid listing-grid-home">
      <article v-for="item in listings" :key="item.id" class="card listing-item listing-item-clickable" @click="openListing(item)">
        ...
        <div v-else class="listing-cover listing-cover-empty">����ͼƬ</div>
        <div class="listing-body">
          ...
          <div class="listing-meta"><span>{{ item.district_name || 'δ��������' }}</span><span>{{ formatRelativeTime(item.created_at) }}</span></div>
          <div class="listing-price-row"><div class="price">{{ formatPrice(item.price) }}</div><div class="listing-stats">{{ formatCompactNumber(item.view_count || 0) }} ���</div></div>
          <div class="listing-seller-row">
            <div class="seller-mini"><span class="seller-mini-avatar">{{ getAvatarText(item.seller_nickname) }}</span><span>{{ item.seller_nickname || '�������' }}</span></div>
            <button class="btn btn-primary btn-mini" @click.stop="openChat(item)">��һ��</button>
          </div>
        </div>
      </article>
        ...
        <div class="publish-grid">
          <div class="publish-main">
            ...
            <div class="field">
              ...
              <div class="publish-image-input"><input class="input" v-model.trim="form.image_input" placeholder="粘贴一张图片地址后点击添�? /><button class="btn btn-dark" type="button" @click="addImage">添加</button></div>
...
const EnhancedPublishPage = {
  setup() {
    ...
    function queuePublishFiles(fileList, source = "picker") {
      ...
      if (remain <= 0) {
        errorText.value = "最多上�?9 张图片�?;"
        return;
      }

      const acceptedFiles = files.slice(0, remain);
      if (acceptedFiles.length < files.length) {
        errorText.value = "最多上�?9 张图片，多余图片已忽略�?;"
      }
      ...
      try {
        ...
        if (source === "paste" && selectedImages.value.length) {
          successText.value = "图片已粘贴，提交时统一上传�?;"
        }
      } catch (error) {
    ...
    <section class="message-index">
      ...
      <section v-else class="message-list">
        ...
        <div v-if="!items.length" class="card empty-state">
          <h3>还没有会�?/h3>
          <p>从商品详情点击“聊一聊”后，会话就会出现在这里�?/p>
          <router-link class="btn btn-primary" to="/">去逛首�?/router-link>
        </div>
      </section>
    </section>
  `,
    ...
    <section v-else-if="detail" class="chat-shell">
      ...
      <footer class="chat-composer card">
        ...
        <div class="chat-input-row">
          ...
          <button class="btn btn-primary chat-send" :disabled="sending" @click="sendMessage">{{ sending ? '发送中...' : '发�? }}</button>
        </div>
      </footer>
    </section>
  `,
...
const EnhancedMessageDetailPage = {
  setup() {
    ...
    function queueComposerFiles(fileList, source = "paste") {
      ...
      if (remain <= 0) {
        setComposerMessage("聊天中最多暂�?9 张待发送图片�?��", "error");
        return;
      }
      ...
      if (addedCount > 0) {
        ...
        setComposerMessage(`${sourceText} ${addedCount} 张图片，点击发送后上传`);
      }
    }
    ...
    async function captureScreenshot() {
      ...
      try {
        ...
        setComposerMessage(
          isAbort
            ? "截图未授权，您也可以直接 Ctrl+V 粘贴截图发送�?"
            : error.message || "截图发送失败，请改用粘贴图片�?",
          "error",
        );
      } finally {
        if (stream) {
    ...
    async function handlePaste(event) {
      ...
      try {
        ...
        setComposerMessage(error.message || "粘贴图片发送失败，请重试�?��", "error");
      }
    }
    ...
    function retryImageMessage() {
      setComposerMessage("失败图片需要重新选择后发送�?��", "error");
    }
              ...
              <div class="chat-bubble" :class="{ mine: isMine(message), 'chat-bubble-image': isImageMessage(message) }">
                <template v-if="isImageMessage(message)">
                  ...
                  <div class="chat-image-meta">
                    ...
                    <span v-else-if="message.local_status === 'failed'" class="chat-message-status chat-message-status-error">{{ message.local_error || '发送失�? }}</span>
      ...
      <footer class="chat-composer card">
        ...
        <div v-if="false && pendingComposerImage" class="chat-composer-attachment">
          ...
          <div class="chat-composer-attachment-meta">
            <strong>待发送图�?/strong>
            <span>{{ pendingComposerImage.source === 'paste' ? '已粘贴，点击发送后上传' : '已选择，点击发送后上传' }}</span>
          </div>
        ...
        <div class="chat-input-row">
          <input
            ...
            @paste="handlePaste"
          />
          <button class="btn btn-primary chat-send" :disabled="sending || !canSend" @click="requestSend('button')">{{ sending ? '发送中...' : '发�? }}</button>
        </div>
      </footer>
  ...
  setup() {
    ...
    async function toggleListingStatus(item) {
      ...
      try {
        ...
        errorText.value = error.message || "更新帖子状态失败�?;"
      } finally {
    ...
    <section class="profile-shell">
      ...
      <section class="card section">
        <div class="section-title-row">
          <h2>我发布的帖子</h2>
          <button class="btn btn-ghost btn-mini" @click="logout">退出登�?/button>
        </div>
        <div class="error" v-if="errorText">{{ errorText }}</div>
  ...
  template: `
    <section class="favorites-shell">
      <section class="card section">
        ...
        <div class="muted favorites-subline">收藏后可快速回到商品详情继续沟通�?/div>
      </section>
  ...
  template: `
    <div class="app-shell" :class="{ immersive: immersiveLayout, auth: authLayout }">
      <header v-if="!immersiveLayout && !authLayout" class="topbar">
        <div>
          <div class="brand">��������</div>
          <div class="brand-subline">������������ �� ������ Web ��</div>
        </div>
      ...
      <nav v-if="!immersiveLayout && !authLayout" class="footer-nav">
        <router-link to="/" :class="{ active: currentTab === 'home' }">��ҳ</router-link>
        <router-link to="/categories" :class="{ active: currentTab === 'categories' }">����</router-link>
        <router-link class="footer-publish" to="/publish" :class="{ active: currentTab === 'publish' }">����</router-link>
        <router-link to="/messages" :class="{ active: currentTab === 'messages' }">��Ϣ</router-link>
        <router-link to="/me" :class="{ active: currentTab === 'me' }">�ҵ�</router-link>
      </nav>
    </div>
  `,
};
