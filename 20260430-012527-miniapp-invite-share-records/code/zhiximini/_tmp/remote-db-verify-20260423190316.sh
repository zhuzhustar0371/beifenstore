...
MYSQL_PWD="$DB_PASSWORD_VALUE" mysql -N -h "$DB_HOST" -P "$DB_PORT" -u "$DB_USER_VALUE" "$DB_NAME" -e "SELECT column_name FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='cashback_records' AND column_name IN ('transfer_detail_id','transfer_fail_reason') ORDER BY column_name;"