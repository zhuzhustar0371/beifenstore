...
import com.zhixi.backend.dto.AdminRefundApproveRequest;
import com.zhixi.backend.dto.AdminRefundRejectRequest;
...
public class AdminController {
  ...
  public ApiResponse<AdminPageResult<AdminCashbackVO>> cashbacks(
    ...
    return ApiResponse.ok(adminManageService.pageCashbacks(userId, type, status, nickname, page, size));
  }
  ...
  @GetMapping("/invites/progress")
  public ApiResponse<AdminPageResult<AdminInviteProgressVO>> inviteProgress(
      @RequestParam(required = false) Long inviterId,
      @RequestParam(required = false) Long inviteeId,
      @RequestParam(required = false) String keyword,
      @RequestParam(required = false) Integer page,
    ...
    return ApiResponse.ok(adminManageService.pageInviteProgress(inviterId, inviteeId, keyword, page, size));
  }
  ...
  @PostMapping("/orders/{orderId}/cancel-shipment")
  public ApiResponse<Void> cancelShipment(@PathVariable Long orderId) {
    adminManageService.cancelShipment(orderId);
    return ApiResponse.ok(null);
  }
  ...
  public ApiResponse<Void> refundOrder(
    ...
    adminManageService.refundOrder(orderId, reason, customAmount, adminId);
    return ApiResponse.ok(null);
  }

  @PostMapping("/orders/{orderId}/refund/approve")
  public ApiResponse<Void> approveRefundRequest(
      @PathVariable Long orderId,
      @RequestBody(required = false) AdminRefundApproveRequest request,
      @RequestHeader(value = "Authorization", required = false) String authorization
  ) {
    BigDecimal amount = request != null ? request.getAmount() : null;
    String remark = request != null ? request.getAdminRemark() : null;
    Long adminId = resolveAdminId(authorization);
    adminManageService.approveRefundRequest(orderId, amount, remark, adminId);
    return ApiResponse.ok(null);
  }
  ...
  public ApiResponse<Void> rejectRefundRequest(
    ...
    String remark = request != null ? request.getReviewRemark() : null;
    Long adminId = resolveAdminId(authorization);
    adminManageService.rejectRefundRequest(orderId, remark, adminId);
    return ApiResponse.ok(null);
  }
  ...
  public ApiResponse<WithdrawalRequest> approveWithdrawal(
    ...
    if (payload != null && payload.get("amount") != null) {
      try {
        ...
        return ApiResponse.fail("批准金额无效");
      }
    }
  ...
  @PostMapping("/invites/reset-all")
  public ApiResponse<Map<String, Object>> resetAllInvites(
      @RequestHeader(value = "Authorization", required = false) String authorization,
      @Valid @RequestBody AdminDangerousActionRequest request
  ) {
    adminAuthService.verifyCurrentAdminPassword(extractToken(authorization), request.getAdminPassword());
    return ApiResponse.ok(adminManageService.resetAllInvites());
  }
  ...
  private Long resolveAdminId(String authorization) {
    try {
      return adminAuthService.getAdminByToken(extractToken(authorization)).getId();
    } catch (Exception e) {
      return null;
    }
  }
