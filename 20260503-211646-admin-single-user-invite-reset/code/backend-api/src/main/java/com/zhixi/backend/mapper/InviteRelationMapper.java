...
public interface InviteRelationMapper {
  ...
  @Select({
      "<script>",
      "SELECT COUNT(1) FROM invite_relations ir",
      "WHERE 1=1",
      "<if test='inviterId != null'>",
      "  AND ir.inviter_user_id = #{inviterId}",
      "</if>",
      "<if test='inviteeId != null'>",
      "  AND ir.invitee_user_id = #{inviteeId}",
      "</if>",
      "<if test='keyword != null and keyword != \"\"'>",
      "  AND (",
      "    CAST(ir.inviter_user_id AS CHAR) LIKE CONCAT('%', #{keyword}, '%')",
      "    OR CAST(ir.invitee_user_id AS CHAR) LIKE CONCAT('%', #{keyword}, '%')",
      "    OR EXISTS (",
      "      SELECT 1 FROM users u",
      "      WHERE u.id = ir.inviter_user_id",
      "        AND u.nickname LIKE CONCAT('%', #{keyword}, '%')",
      "    )",
      "    OR EXISTS (",
      "      SELECT 1 FROM users u",
      "      WHERE u.id = ir.invitee_user_id",
      "        AND u.nickname LIKE CONCAT('%', #{keyword}, '%')",
      "    )",
      "    OR EXISTS (",
      "      SELECT 1 FROM user_wechat_auth uwa",
      "      WHERE uwa.user_id = ir.inviter_user_id",
      "        AND uwa.nickname LIKE CONCAT('%', #{keyword}, '%')",
      "    )",
      "    OR EXISTS (",
      "      SELECT 1 FROM user_wechat_auth uwa",
      "      WHERE uwa.user_id = ir.invitee_user_id",
      "        AND uwa.nickname LIKE CONCAT('%', #{keyword}, '%')",
      "    )",
      "  )",
      "</if>",
  ...
  long countByAdminProgressQuery(
      @Param("inviterId") Long inviterId,
      @Param("inviteeId") Long inviteeId,
  ...
  @Select({
      "<script>",
      "SELECT ir.id, ir.inviter_user_id AS inviterId, ir.invitee_user_id AS inviteeId, ir.bind_time AS boundAt, ir.first_paid_time AS firstPaidAt",
  ...
  List<InviteRelation> findByAdminProgressQuery(
      @Param("inviterId") Long inviterId,
      @Param("inviteeId") Long inviteeId,
      @Param("keyword") String keyword,
      @Param("offset") int offset,
