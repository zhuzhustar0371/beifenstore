...
export async function fetchAdminInviteProgress(params = {}) {
  const { data } = await client.get("/admin/invites/progress", { params });
  const page = data.data || {};
  return {
    records: page.records || [],
    total: Number(page.total || 0),
...
export async function cancelShipment(orderId) {
  await client.post(`/admin/orders/${orderId}/cancel-shipment`);
}
...
export async function approveRefundRequest(orderId, amount, adminRemark) {
  const payload = {};
  if (amount !== undefined && amount !== null && amount !== "") {
    payload.amount = Number(amount);
  }
  if (adminRemark !== undefined && adminRemark !== null) {
    payload.adminRemark = String(adminRemark);
  }
  await client.post(`/admin/orders/${orderId}/refund/approve`, payload);
}

export async function rejectRefundRequest(orderId, reviewRemark) {
  const payload = {};
  if (reviewRemark !== undefined && reviewRemark !== null) {
    payload.reviewRemark = String(reviewRemark);
  }
  await client.post(`/admin/orders/${orderId}/refund/reject`, payload);
}
...
export async function resetAllInvites(adminPassword) {
  const { data } = await client.post("/admin/invites/reset-all", { adminPassword });
  return data.data;
}
